...
impl ClassifierPlot {
  pub fn new(model: Model, clouds: Vec<Cloud>) -> ClassifierPlot {
    ClassifierPlot { model, clouds }
  }

  // Render the plot, with each cloud coloured according to class
  pub fn render(&self) -> Result<(), Box<dyn std::error::Error>> {
    // Delete existing plot and file if they exist
    std::fs::remove_file("plotters-doc-data/PlotPredict.png").unwrap_or(());
    std::fs::remove_file("Data.txt").unwrap_or(());

    // Drawing area and file for points
    let root = BitMapBackend::new("plotters-doc-data/PlotPredict.png", (640, 480)).into_drawing_area();
    let mut file = File::create("Data.txt").unwrap();
